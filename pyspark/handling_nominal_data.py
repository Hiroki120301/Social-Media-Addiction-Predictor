...
df = spark.read.csv(
    "CSDS-312-Final-Project/pyspark/Data/dataset.csv", header=True, inferSchema=True)
categoricalCols = ['Age', 'Gender',
                   'Relationship Status', 'Occupation', 'Hours Per Day']
indexers = [StringIndexer(inputCol=col, outputCol=col+"_index",
                          handleInvalid="keep") for col in categoricalCols]
encoders = [OneHotEncoder(inputCol=col+"_index",
                          outputCol=col+"_encoded") for col in categoricalCols]
